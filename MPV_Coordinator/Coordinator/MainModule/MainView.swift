...
final class MainView: UIView {
    ...
    lazy var runSecondPageButton: UIButton = {
        let button = UIButton()
        button.translatesAutoresizingMaskIntoConstraints = false
        button.setTitle("Next page", for: .normal)
        button.setTitleColor(.systemPink, for: .highlighted)
        button.addTarget(self, action: #selector(didPressedSecondPageButton), for: .touchUpInside)
        return button
    }()
    ...
    override init(frame: CGRect) {
        ...
        addSubview(runSecondPageButton)
        setupConstraints()
    }
    ...
    private func setupConstraints() {
        NSLayoutConstraint.activate([
            ...
            runSecondPageButton.topAnchor.constraint(equalTo: clearButton.bottomAnchor, constant: 20),
            runSecondPageButton.leadingAnchor.constraint(equalTo: leadingAnchor, constant: 20),
            runSecondPageButton.trailingAnchor.constraint(equalTo: trailingAnchor, constant: -20)
        ])
    }
    ...
    @objc func didPressedSecondPageButton() {
        delegate?.runSecondPageDidPressed()
    }
}
